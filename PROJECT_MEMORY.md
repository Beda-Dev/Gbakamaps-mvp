...
- **Gestion CRUD admin complète** (demande explicite, confirmée deux fois par l'utilisateur, "je le veux aussi, garde ça en tête") : au-delà de la modération des signalements et du tarif des lignes (déjà backend uniquement, §12.7), l'utilisateur veut pouvoir gérer plus largement les données depuis le front en tant qu'admin (ajouter/modifier/supprimer arrêts, lignes, etc.). Rien construit à ce stade au-delà de `PATCH /api/admin/lines/:id` (backend seul, pas d'écran) — un vrai chantier à part, à concevoir (quelles entités, quelles actions, quels garde-fous) avant d'implémenter. **Toujours en attente.**
