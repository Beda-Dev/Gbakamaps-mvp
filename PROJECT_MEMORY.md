...
### 12.18 Approximation "zone accessible en X minutes" (backend) — livré le 2026-09-08
...
**Reste à faire (frontend, non commencé)** : mode/toggle sur la carte principale — clic sur un point (ou position utilisateur) → surlignage des arrêts atteignables dans le budget choisi (15/30/45 min). Enveloppe convexe seulement si une lib légère est déjà présente, sinon simple surlignage (plus honnête qu'un faux polygone lissé). Point de synchro fait avec `gbakamaps-8a` après le backend, comme demandé.
