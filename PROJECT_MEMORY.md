...
### 12.23 Correspondance par courte marche entre arrêts proches — dernier des 3 chantiers de §12.16 — livré le 2026-09-09

Extension de `planTrip` (`trip-planning.service.ts`) : une correspondance a désormais lieu au MÊME arrêt physique (comportement d'origine, inchangé) OU via une **courte marche** (≤ `TRANSFER_WALK_RADIUS_METERS = 300`, choix produit documenté — pas une mesure) vers un arrêt proche desservi par une autre ligne. Un SEUL signal (la distance) suffit ici, contrairement au rapprochement de lieux (§12.7bis, double signal proximité+nom exigé en l'absence d'ID commun) : il ne s'agit pas de décider si deux entités désignent la même chose, mais de répondre à une question géométrique — "existe-t-il un arrêt d'une autre ligne assez proche pour y marcher ?".

**Implémentation** : l'ancienne recherche `secondLegByTransferStop.get(transferEntry.stopId)` (correspondance strictement au même `stopId`) étendue en une liste `secondLegBoardingPoints` de TOUS les points d'embarquement de seconde jambe connus, comparée par distance (haversine, en mémoire — aucune requête DB supplémentaire) à CHAQUE arrêt d'arrivée de première jambe. Distance nulle pour un même `stopId` (aucun pas de marche inséré, comportement identique à avant) ; sinon un vrai step `walk` inséré ENTRE les deux `ride`, avec `fromLabel`/`toLabel` nommant les deux arrêts.

**Bug de conception trouvé en écrivant les tests, pas en théorie** : la première version comparait aussi les arrêts INTERMÉDIAIRES d'une ligne (pas seulement son terminus de correspondance) — ex. descendre un arrêt trop tôt puis marcher en diagonale peut être authentiquement plus rapide que rester à bord jusqu'au bon arrêt, si la géométrie s'y prête. Ce n'est PAS un bug de logique (un vrai planificateur de trajet doit considérer ce cas), mais un piège pour les tests synthétiques : la topologie de test initiale (arrêts espacés de 150-200m) plaçait involontairement le nouvel arrêt de test E à moins de 300m de PLUSIEURS arrêts de lignes différentes à la fois (B ET C, D ET C), créant des plans parasites qui gagnaient le tri "le plus rapide" sans rapport avec le scénario voulu. Corrigé en recalculant précisément (script Node, pas au jugé) des coordonnées de test où seul le point de correspondance intentionnel (C) est à moins de 300m du nouvel arrêt E, tous les autres arrêts existants (A, B, D) au-delà.

**Vérifié réellement** : `tsc --noEmit` OK ; backend 12/12 `trip-planning.test.ts` (3 nouveaux : correspondance par courte marche trouvée avec un vrai step `walk` intercalé de distance cohérente, aucune correspondance proposée si `maxTransfers=0`, note mise à jour) + 9/9 déjà existants (aucune régression, y compris les 2 tests de désactivation de ligne du même fichier) ; 142/146 tests backend au total (4 échecs = flakiness réseau Overpass déjà documentée, module `places` non touché) ; **testé contre les vraies données Abidjan** après rebuild Docker : trajet réel Institut des Aveugles → Paroisse St André retourne une correspondance à pied authentique de 271m entre "Bel Air Arret Cine Cool" (ligne 729) et "Rue princesse" (ligne 47) — un cas réel que l'ancienne version aurait manqué.

**Note du planificateur mise à jour** : "Correspondance au même arrêt ou via une courte marche (300 m maximum) vers un arrêt proche d'une autre ligne" remplace l'ancienne mention "limitées à un changement au même arrêt physique", devenue fausse.

**Les 3 chantiers de §12.16 (délégation `gbakamaps-f4`, jamais aboutie) sont désormais tous terminés** : CRUD lignes (§12.22), onboarding (fait par Cline, §12.17), correspondance par courte marche (ici). Reste, hors de ce lot initial : croisement fin des arrêts SOTRA restants au-delà du matching par ID exact (§12.20) — piste identifiée, pas encore engagée.

### 12.24 Audit UX/UI/accessibilité — démarré le 2026-09-09

Demande explicite et détaillée de l'utilisateur : explorer réellement l'app (pas en théorie) et corriger ce qui peut l'être, en s'inspirant des standards Google Maps — UX/UI, accessibilité, CRUD visuel, drag & drop, icônes, animations, états (loading/empty/error/success/disabled/selected/hover/focus), responsive, workflows, cohérence visuelle. **Chantier volontairement engagé par passes successives, pas en un seul lot** — l'ampleur (toute l'application, tous les états) dépasse ce qu'une seule itération peut couvrir sérieusement sans survoler ; chaque passe suivante doit partir d'un audit réel (captures Playwright réelles, desktop+mobile+tablette), pas d'une supposition.

**Première passe réelle effectuée** (Edge/Playwright headless, captures desktop 1440px / mobile 390px / tablette 820px, sur `/`, `/login`, `/signup`, `/planifier`, `/favorites`, `/reports`) :

**Bug réel trouvé et corrigé** : sur mobile (`/planifier`), un grand vide gris (~135px) séparait la carte du formulaire. Cause identifiée précisément (pas au jugé, via inspection des hauteurs calculées réelles) : `.trip-planner__map-panel` gardait `flex: 1` (hérité du layout desktop, où le panel carte doit occuper l'espace à côté du formulaire) même dans la media query mobile — en colonne empilée, `flex:1` étire le panel sur TOUTE la hauteur restante de l'écran, alors que la carte à l'intérieur garde sa hauteur fixe (260px), laissant le reste vide. Corrigé (`flex: none` dans la media query `max-width:860px`) et **vérifié par mesure réelle** : hauteur du panel passée de 394,89px à exactement 260px (celle de la carte), capture d'écran de confirmation sans vide résiduel.

**Défauts identifiés, pas encore corrigés (à traiter dans une prochaine passe)** :
- Sur `/` mobile, un cluster important ("20" arrêts) apparaît partiellement masqué sous la barre du bas (rayon + boutons filtres), fond blanc opaque — pas un bug fonctionnel (le cluster reste cliquable en zoomant) mais un vrai défaut visuel. Piste : padding de viewport MapLibre (`fitBounds`/`easeTo` avec `padding`) pour que la zone "utile" de la carte tienne compte de la hauteur de cette barre, plutôt qu'un simple ajustement CSS qui ne réglerait que l'apparence.
- Le sélecteur de style de carte (5 boutons texte "Rues/Basique/Plein air/Satellite/Hybride") reste dense sur petit mobile (le dernier bouton frôle le bord à 390px) — fonctionnel grâce à `flex-wrap`, mais pas au niveau d'un menu compact façon Google Maps (icône unique "calques" ouvrant un sélecteur). Piste identifiée, pas encore implémentée : nécessite un nouveau composant (popover/menu), pas un simple ajustement CSS.
- Page `/planifier` desktop : beaucoup d'espace vide sous le panneau formulaire tant qu'aucune recherche n'est lancée — pas un bug, mais un manque de guidage visuel pour un premier visiteur.

**Pages non encore auditées en profondeur** (nécessitent un compte connecté pour voir leur vrai contenu, pas seulement la redirection `/login`) : `/favorites`, `/reports`, `/admin/*` (stops/lines/reports) à l'état rempli, panneau de détail d'un arrêt, formulaire de signalement, galerie photos, narration IA, flux de connexion complet. À couvrir dans une prochaine passe avec une session authentifiée réelle (comme pour la vérification d'AdminLinesPage, §12.22).

**Vérifié réellement** : `tsc`/tests inchangés (98/98 frontend, correctif CSS pur, aucune logique touchée), capture avant/après comparée pixel-à-pixel (vide disparu, hauteur du panel mesurée en JS dans la page).

**Deuxième passe (2026-09-09)** — sélecteur de style + audit avec session connectée :

- **Sélecteur de style de carte** : sur petit écran (≤480px), remplacé les 5 boutons texte par un bouton unique "calques" (nouvelle icône `LayersIcon`, Lucide) qui ouvre un menu déroulant — façon Google Maps, demande explicite de l'utilisateur. Desktop inchangé (rangée toujours visible). Nouveau state `styleMenuOpen` dans `StopsMap.tsx`, se referme automatiquement après sélection.
- **Cluster "caché" réexaminé, PAS un vrai bug** : vérifié par mesure réelle du DOM (`getBoundingClientRect`) que le canvas MapLibre s'arrête EXACTEMENT au bord de la barre du bas (0px de chevauchement) — l'impression de recouvrement dans la première capture était un artefact (bannière d'onboarding pas encore fermée au moment du screenshot). Important de le dire : ne pas corriger un problème qui n'existe pas, même identifié à tort dans une passe précédente.
- **Session admin de test créée directement en base** (comme les tests d'intégration — jamais de mot de passe deviné pour un vrai compte), avec un favori et un signalement réels semés, pour auditer `/favorites`, `/reports`, `/admin/reports`, `/admin/stops`, `/admin/lines` à l'état REMPLI (pas juste la redirection login vue lors de la première passe).
- **Bug réel trouvé et corrigé (`FavoritesPage.tsx`)** : le type d'arrêt s'affichait en valeur d'enum brute ("BUS_STOP") au lieu du libellé français déjà utilisé partout ailleurs (`STOP_TYPE_LABELS`, déjà exporté par `StopsMap.tsx` et utilisé dans le panneau détail de `HomePage.tsx`) — incohérence de traduction, corrigée par réutilisation directe du même mapping (jamais un nouveau mapping dupliqué).
- **Bug réel trouvé et corrigé (`AdminLinesPage.tsx`, mobile)** : les 3 actions (Modifier/Désactiver/×) forçaient le nom de la ligne — l'information la plus importante de la carte — sur une colonne trop étroite, tronqué en "02 — Cité Fairmon…" illisible (`text-overflow:ellipsis` déjà en place, mais plus assez de place). Corrigé en enveloppant les 3 actions dans un nouveau conteneur `.admin-lines__item-actions`, qui passe sur sa propre rangée en dessous du nom (pleine largeur) sous 480px — le nom s'affiche désormais toujours en entier, sur plusieurs lignes si besoin.
- **Défaut identifié, pas corrigé** : `AdminStopsMap` (carte de gestion des arrêts) n'a pas de clustering — en zone dense (Plateau), les marqueurs individuels (nécessaires pour le glisser-déposer, §12.15) s'empilent en un amas illisible. Contrairement à la carte publique (clustering GeoJSON, §12.11), impossible d'appliquer le même mécanisme tel quel sans perdre le drag&drop par marqueur individuel — chantier à concevoir séparément (ex. dé-clustering progressif au zoom, ou clustering desactivé seulement en mode "sélection multiple").
...
### 12.26 Filtre par mode de transport sur la carte principale — livré le 2026-09-09

Remarque directe de l'utilisateur : *"j'ai l'impression que les gbaka sont un peu negligé"*. Vérifié en base AVANT de conclure quoi que ce soit (jamais une impression non vérifiée) : les données gbaka ne sont PAS pauvres — **1103 arrêts GBAKA_STOP** (plus que WORO_WORO_STOP : 1089), **109 lignes GBAKA** (BUS 112, WORO_WORO 170), couleur (`#EE9B00`) et libellé ("Gbaka") déjà distincts partout où un arrêt est affiché. Le vrai écart trouvé : `stopModeEnum`/`modes=gbaka,woroworo,taxi,mototaxi` existait déjà côté API (`stops.schemas.ts`), mais **aucun hook ni composant frontend ne l'utilisait** — impossible pour un utilisateur de filtrer la carte par mode, quel qu'il soit (pas spécifique aux gbaka).

**Frontend** : `useNearbyStops(lat, lon, radius, modes?)` accepte désormais un 4e paramètre optionnel (tableau de modes, trié avant d'entrer dans la `queryKey` pour rester stable quel que soit l'ordre de sélection de l'utilisateur) — absent, comportement strictement inchangé (aucun paramètre `modes` envoyé). `HomePage.tsx` : nouvelles chips `home__mode-filters` sous la barre de comptage existante ("X arrêts dans un rayon de…"), une par mode (Gbaka/Wôrô-wôrô/Taxi/Moto-taxi), colorées avec les couleurs réelles déjà utilisées sur la carte (`STOP_TYPE_COLORS`, jamais une nouvelle palette inventée) — sémantique OU côté backend (cocher Gbaka+Taxi montre les deux, pas leur intersection).

**Vérifié réellement** : `tsc --noEmit` OK, 100/100 tests frontend (2 nouveaux `useNearbyStops.test.tsx` : sans mode le paramètre reste absent, avec modes il est trié et transmis). **Vérifié en navigateur réel et contre l'API réelle** (pas seulement visuellement) : activer le filtre "Gbaka" sur le Plateau fait passer le compteur de 101 à 13 arrêts, confirmé indépendamment par deux appels `curl` directs (`/stops/nearby` avec et sans `modes=gbaka`) — la carte affiche alors uniquement les points orange gbaka. Fonctionne identiquement en desktop et mobile (chips scrollables horizontalement, aucun chevauchement avec les autres contrôles).

### 12.27 Audit technique/sécurité/infra complet — livré le 2026-09-09

Demande explicite et très détaillée de l'utilisateur : audit sans complaisance de l'ensemble du projet (architecture, sécurité, Docker, DevOps, logique métier, DB, tests), avec un regard "nouveau lead technique". Rapport complet livré dans la conversation (pas dupliqué intégralement ici) ; ce paragraphe documente les correctifs RÉELLEMENT appliqués et vérifiés pendant l'audit (les autres findings restent des recommandations, pas des changements de code).

**🔴 Corrigé — port Postgres exposé sur toutes les interfaces** : `docker-compose.yml` publiait `${POSTGRES_PORT}:5432` sans préfixe d'adresse, donc sur `0.0.0.0` — sur un vrai serveur (jamais testé en conditions réelles jusqu'ici), la base aurait été directement joignable depuis Internet, les identifiants devenant la seule protection. Corrigé en `127.0.0.1:${POSTGRES_PORT:-5432}:5432` — le backend continue d'atteindre `db` par le réseau Docker interne, seul l'accès direct depuis l'hôte externe est restreint. Vérifié : `docker port` confirme le nouveau binding.

**🟠 Corrigé — aucun HEALTHCHECK Docker** : un vrai endpoint `/api/health` existe (vérifie la DB, pas juste "le process tourne") mais rien ne l'appelait côté Docker — `depends_on`/`docker compose ps` ne pouvaient jamais détecter un backend bloqué (event loop gelé, DB injoignable) tant que le process restait vivant. Ajouté dans `Dockerfile` (HEALTHCHECK, `wget` déjà présent dans `node:22-alpine` — vérifié, `curl` absent) et reflété dans `docker-compose.yml`. Vérifié : `docker inspect --format='{{.State.Health.Status}}'` renvoie `healthy`.

**🟠 Corrigé — aucun en-tête de sécurité HTTP** : ni CSP, ni `X-Content-Type-Options`, ni `X-Frame-Options`, ni HSTS. Ajouté `@fastify/helmet` (config par défaut — cette API ne sert que du JSON + des photos statiques, jamais de HTML/JS applicatif, donc la CSP par défaut ne casse rien). Vérifié par `curl -I` réel : tous les en-têtes attendus présents.

**🟠 Corrigé, avec une vraie régression trouvée ET réparée en cours de route — conteneur backend tournant en root** : aucun `USER` défini dans le `Dockerfile`, confirmé par `docker exec ... id` (`uid=0`). Un premier correctif naïf (`USER node` juste après un `chown` dans le Dockerfile) a semblé fonctionner sur un volume neuf, mais **cassait réellement l'upload de photos communautaires sur le volume de dev existant** (`EACCES`, reproduit et confirmé par un vrai upload avant/après) — ce volume avait été créé du temps où le conteneur tournait encore en root, donc restait `root:root` même après le changement d'image. Corrigé proprement avec un entrypoint (`docker-entrypoint.sh`, `su-exec`) qui démarre en root, corrige la propriété du volume (`chown`, idempotent), PUIS lance le process réel en tant que `node` — jamais le serveur Node lui-même en root, y compris sur un déploiement déjà existant. Vérifié trois fois : (1) `docker top` confirme `node dist/server.js` tourne en `uid=1000`, (2) un vrai upload de photo réussit sur le volume EXISTANT (root:root avant), (3) un environnement entièrement neuf (`docker volume rm` + `docker compose up`) fonctionne aussi correctement dès le départ.

**Findings documentés mais PAS corrigés dans cette session** (recommandations, pas des bugs — nécessitent soit une décision produit, soit un chantier plus large que le temps disponible ne permettait pas de traiter sûrement) :
- CI ne lance jamais la suite backend (vraie DB PostGIS requise) — recommandation : ajouter un service `postgres` à la CI GitHub Actions pour au moins les tests n'appelant aucune API externe, garder les autres exclus.
- `packages/shared-types` déclaré dans les workspaces racine mais totalement vide (0 fichier) — soit le supprimer, soit l'implémenter réellement (les types sont dupliqués à la main entre front/back, décision déjà assumée et documentée ailleurs).
- Vérification "Origin absent" fail-open sur la défense CSRF applicative (`app.ts`) — `SameSite=Lax` reste la vraie protection, ce garde-fou n'est qu'une défense en profondeur ; resserrer son comportement demande de vérifier d'abord qu'aucun consommateur légitime (mobile natif ?) n'envoie de mutation authentifiée sans en-tête `Origin`.
- Pas de scan de contenu réel (magic bytes) sur les photos uploadées — seul le `Content-Type` déclaré par le client est vérifié (extension forcée côté serveur ensuite, donc pas d'exécution de script possible, mais un fichier non-image pourrait être accepté sous une extension image).
- Aucun job planifié de nettoyage des sessions expirées non réutilisées (le commentaire d'`auth.service.ts` mentionnait "le cron sert juste le ménage" — **aucun cron n'existe réellement dans ce projet**, trouvé en cherchant ; l'expiration reste effective à l'usage, seule la purge de lignes mortes en base est absente).
- `npm install` (pas `npm ci`) dans le `Dockerfile`, et devDependencies incluses dans l'image finale (déjà documenté §9 comme risque connu, non repris ici).

**Vérifié réellement au global** : 145/151 tests backend (6 échecs = flakiness Overpass déjà documentée, aucun rapport avec ces changements), 102/102 tests frontend, `tsc --noEmit` propre des deux côtés, `prisma migrate status` confirme aucune dérive schéma/migrations.
