...
**⚠️ Pollution de données réelle laissée par les tests manuels** : un arrêt réel importé d'OSM a été renommé "Test rename" et déplacé pendant la vérification manuelle (curl + glisser-déposer réel) — pas nettoyé (position exacte d'origine non mémorisée avec certitude). Sans conséquence fonctionnelle en dev, mais à corriger si un ré-import GTFS/OSM propre est fait un jour, ou à identifier/renommer manuellement si remarqué sur la carte principale.
...
**Découverte de coordination pendant cette session** : trois nouvelles sessions peer sont apparues (`gbakamaps-10`, `gbakamaps-82`, en plus de `gbakamaps-f4` déjà en cours) suite à la demande de l'utilisateur de déléguer "à Cline et OpenCode" — toutes se sont révélées être des sessions Claude Code standard démarrant par défaut sur l'ancien repo `GbakaMaps`, pas des instances Cline/OpenCode distinctes (ces outils ne sont pas joignables via le même canal de messagerie inter-sessions). `gbakamaps-82` a reçu la délégation du câblage frontend (galerie photos + bouton narration IA) pour ne pas dupliquer le travail de `gbakamaps-f4` (CRUD lignes) ni `gbakamaps-10` (en attente d'affectation).
